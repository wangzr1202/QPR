...
class Monitor(object):
    ...
    def macro_auc_roc(self):
        """
        计算 macro-averaged ROC-AUC score。

        Args:
            y_true (array-like): 真实标签，形状为 (n_samples, n_classes)。
            y_pred (array-like): 预测概率或置信度，形状为 (n_samples, n_classes)。

        Returns:
            float: macro-averaged ROC-AUC score。
        """
        n_classes = self.ytrue_record.shape[1]  # 获取类别数量
        auc_scores = []
        
        for i in range(n_classes):
            # 如果某一类别的标签全为正类或负类，则跳过（无法计算 ROC-AUC）
